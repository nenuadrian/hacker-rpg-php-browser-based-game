<?php foreach($services as $service_id => $service): ?>
	<?php if ($service['quest_server_id'] != $server_id) continue; $type = Config::get('service_types')[$service['type']];?>
	...
	<div class="collapse well <?php echo $service_expanded == $service_id ? 'in' : ''; ?>" id="service_<?php echo $service_id; ?>" >
		...
		<a data-toggle="modal" data-target="#service-<?php echo $service_id; ?>" class="btn btn-block">edit</a>
			...
			<?php foreach($entities as $entity_id => $e): if ($e['service_id'] != $service_id) continue; ?>
				<button id="anchor_entity_<?php echo $entity_id; ?>" class="btn btn-default btn-block" data-toggle="collapse" data-target="#entity_<?php echo $entity_id; ?>">
		  			<?php echo $e['title']; ?> | <?php echo $e['owner']; ?>  | S<?php echo $e['security']; ?> | <?php echo $entity_id; ?>
				</button>
				<div class="collapse well <?php echo in_array($entity_id, $entity_expanded) ? 'in' : ''; ?>" id="entity_<?php echo $entity_id; ?>" >
					...
					<div class="row">
						...
						<?php foreach(explode(';', html_entity_decode($service['users'], ENT_QUOTES)) as $u): $u = explode(':', $u)[0];?>
							<option value="<?php echo htmlspecialchars($u); ?>" <?php echo $u == $e['owner'] ? 'selected' : ''; ?>><?php echo ($u); ?></option>
						<?php endforeach; ?>
						</select>
						</div>
						<div class="col-xs-4">
