            ...
            if (!$task['remaining']) {
                ...
                if ($task['server_id']) {
                    if ($task['task_type'] == 6) {
                        ...
                        $profit = 2;
                        \DB::update('users')->set(array('money' => \DB::expr('money + ' . $profit)))->where('id', $task['user_id'])->limit(1)->execute();
                    ...
                    $user = \DB::select('knowledge', 'skills')->from('users')->where('id', $task['user_id'])->limit(1)->execute()->as_array()[0];
